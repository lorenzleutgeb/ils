...
\begin{itemize}
	...
	f(X,Y) := g(X,Y) + h_G(X,Y).
	$$
	Here, $g(X,Y)$ represents the cost required to reach the neighbour $(X,Y)$ from the actual room, while $h_G(X,Y) := h(X,Y,O,X_G,Y_G)$, where $(X_G,Y_G)$ is the location of the goal, $O$ is the orientation that the agent has when reaching $(X,Y)$ from the actual location and $h$ is the function defined in~\ref{eqn:heur}.
\end{itemize}
