...
%DONE Define what \htw is, all its rules and constraints.
%DONE Mention that it is also an example in the standard textbook AIMA and thus suited as an exercise in logic-based AI.
...
The game is set in the world of Wumpus, a square, grid-shaped dungeon: each room of the grid is connected by passageways with the orthogonally adjacent rooms.
The main goal of the player is to find the gold, hidden in one of the rooms, and leave the dungeon alive. 
...
% DONE? explicitly refer to SAT and SMT.
...
\paragraph{\enquote{Explore}.} The agent is in \emph{explore} mode whenever the position of the gold is unknown and safe rooms that are reachable and have not been explored are in the dungeon.
In this mode, the agent moves throughout the dungeon, according to a specific search strategy.
...
In this case, the agent heads back to the initial position to climb up from the dungeon, to end the game.
...
% The vertexes of such a graph represent those rooms lying in the \emph{frontier} of the agent, that is, safe and reachable rooms that are yet to be explored; the edges represent direct connections between rooms.
...
% To compute the minimal cost needed to reach the target room the agent performs an .%s : \(f(n) = g(n) + h(n)\). \
...
%DONE? How does our agent explore the world (A*-search)
...
%DONE? Short section where we describe the system, referring to the appendix (see below)
%TODO as well as that we used DLV.
%TODO Generate and include the Dependency Graph
%DONE Autopilot (if it works at some point)
%TODO Refer to usage file for actually running the system.
%TODO mention WSU.
%DONE mention ASP-Lite
...
% DONE Mention that we digressed into dependency graphs, alternate syntax
% DONE Mention bad/1 and why we introduced it instead of looking at an inconsistent program
